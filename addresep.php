...
<input type="hidden" name="pengguna_id" value="<?= htmlspecialchars($pengguna_id) ?>">
...
<script>
  window.addEventListener("DOMContentLoaded", function () {
    ...
    popup.innerHTML = `
      ...
      ">
        <p style="margin-bottom: 20px; font-size: 18px;">Resep berhasil disimpan!</p>
        <button id="popup-oke" style="
          background-color: #ff7043;
          color: white;
          border: none;
          padding: 10px 20px;
    ...
    // Tangani klik tombol "Oke"
    const okeButton = popup.querySelector("#popup-oke");
    okeButton.addEventListener("click", function () {
      document.body.removeChild(popup);         // Hapus popup
      form.reset();                             // Reset form
      window.history.replaceState({}, "", "addresep.php"); // Hapus ?success=1 dari URL
    });
  });
</script>
