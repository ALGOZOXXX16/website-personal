...
$kategoriId = isset($_GET['kategori_id']) ? intval($_GET['kategori_id']) : null;
...
if ($search !== '') {
    $conditions[] = "resep.judul LIKE '%$search%'";
}

if ($kategoriId !== null) {
    $conditions[] = "resep.kategori_id = $kategoriId";
}
...
<header>
  ...
  <div class="nav-container">
    <form class="search-form" action="#" method="GET">
      <input type="text" name="q" placeholder="Cari resep..." value="<?= htmlspecialchars($_GET['q'] ?? '') ?>" />
...
<div class="banner">
  <?php if ($loggedIn): ?>
    Selamat Datang di Dapur Rasa, <?= htmlspecialchars($username) ?>! 🙋‍♂️
  <?php else: ?>
    Selamat Datang di Dapur Rasa!
  <?php endif; ?>
</div>
...
<h2 style="
  text-align: center;
  margin: 0 0 20px;  /* ubah dari 50px 0 20px */
  color: white;
  font-size: 32px;
  background-color: #ff7043;
  padding: 20px 0;
...
<div class="resep-container">
 <?php if ($hasil->num_rows > 0): ?>
  <?php while ($row = $hasil->fetch_assoc()): ?>
    <a href="detailresep.php?id=<?= $row['resep_id']; ?>&asal=semua.php">
      <div class="resep-card" style="cursor: pointer;">
  ...
  <?php endwhile; ?>
<?php else: ?>
  <p style="text-align: center; font-size: 18px; color: #888; padding: 30px;">
    Resep yang kamu cari tidak ada 🤷
  </p>
<?php endif; ?>
</div>
</div>
